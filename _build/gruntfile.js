module.exports = function(grunt) {
	...
	var initConfig = {
		...
		sass: {
			dist: {
				...
				files: {
					'<%= dirs.theme %><%= dirs.assets %><%= dirs.css %>main.min.css': '<%= dirs.scss %>main.scss'
				}
			},
			dev: {
				options: {
	...
	grunt.registerTask('default', ['sass:dev', 'cssmin', 'growl:sass', 'growl:watch', 'watch']);
	grunt.registerTask('build', ['clean:prebuild', 'bower', 'copy', 'sass:dist', 'concat', 'uglify', 'growl:sass']);
